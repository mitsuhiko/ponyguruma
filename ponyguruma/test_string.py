# -*- coding: euc-jp -*-
"""
    ponyguruma.test_string
    ~~~~~~~~~~~~~~~~~~~~~~

    Port of the Oniguruma bytestring test suite.

    :copyright: Copyright 2007 by Georg Brandl.
    :license: BSD.
"""

from ponyguruma import *
from ponyguruma._constants import ENCODING_EUC_JP

errors = []
runs = [0]

def xx(pattern, str_, from_, to, mem, not_):
    try:
        r = Regexp(pattern, encoding=ENCODING_EUC_JP)
        m = r.search(str_)
        runs[0] += 1
        if not_:
            if not m:
                ...
                errors.append("%r should not match with %r" % (pattern, str_))
        else:
            if m:
...
def x2(pattern, str_, from_, to):
    xx(pattern, str_, from_, to, 0, False)

def x3(pattern, str_, from_, to, mem):
    xx(pattern, str_, from_, to, mem, False)

def n(pattern, str_):
    xx(pattern, str_, 0, 0, 0, True)

if __name__ == '__main__':
  # copied from testc.c from here, but changed "\?" to "?" >>>
  x2("", "", 0, 0);
  x2("^", "", 0, 0);
  x2("$", "", 0, 0);
  x2("\\G", "", 0, 0);
